...
(async () => {
  ...
  await test("peak mode shortens output budget and history", async () => {
    const calls = [];
    const g = createGuide({
      getKey: () => "sk-test",
    ...
    assert.strictEqual(calls[0].max_tokens, 180);
    assert.strictEqual(calls[0].messages.filter((m) => m.role === "user").length, 1);
  });
